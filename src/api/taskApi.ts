...
export const taskApi = {
    getAllTasks: async ({search_id, search, status, sortBy, order = 'desc'}: getAllTasksParams) => {
        try {
            const {data} = await axios.get<ApiResponse>(`${BASE_URL}/tasks`, {params: {search_id, search, status, sortBy, order}})
            return data.result
        } catch (error) {
